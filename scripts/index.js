...
function getPlace(elementData) {
  ...
  const placeLikeButton = createPlace.querySelector('.element__place_like-button');
...
// Ниже - открываем попап редактирования профиля (попап №1) добавляя попапу класс popup_opened в html разметку -------------------------
...
const onoverlay = document.querySelectorAll('.popup')
function closePopupOnOverlay() {
  onoverlay.forEach((popup) => {
...
//ниже - регистрируем обработчик события на конпку закрытия попапа, при клике - вызываем функцию закрытия. 
...
popupForm.addEventListener('submit', changeTextProfile)//регистрация обработчика события
